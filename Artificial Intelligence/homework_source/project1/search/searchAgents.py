    ...
    def getSuccessors(self, state: Any):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if not hitsWall:
                ...
                #Next state is a tuple whose element can't be changed once be initialized
...
def foodHeuristic(state: Tuple[Tuple, List[List]], problem: FoodSearchProblem):
    ...
    for food in foodGrid.asList():
        tempManhattanDistance=abs(position[0]-food[0])+abs(position[1]-food[1])
        if maxManhattanDistance<tempManhattanDistance:
            maxManhattanDistance=tempManhattanDistance
    ...
    for food1 in foodGrid.asList():
        for food2 in foodGrid.asList():
            if not (food1[0]==food2[0] and food1[1]==food2[1]):
                manhattanDistanceTemp=abs(food1[0]-food2[0])+abs(food1[1]-food2[1])
                if manhattanDistanceTemp>maxManhattanDistance:
                    maxManhattanDistance=manhattanDistanceTemp
                    toFood1=abs(position[0]-food1[0])+abs(position[1]-food1[1])
                    toFood2=abs(position[0]-food2[0])+abs(position[1]-food2[1])
    return min(toFood1,toFood2)+maxManhattanDistance
